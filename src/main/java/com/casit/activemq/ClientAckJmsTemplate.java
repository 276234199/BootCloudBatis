...
* 类说明:客户端手动应答模板用
* @author zhouhai
* @version 创建时间：2018年12月19日 下午3:14:39
*/
public class ClientAckJmsTemplate  extends JmsTemplate {
    ...
    /**
     * ack然后关闭
     * 关闭consumer和sesseionToClose是父类本身就要执行的,这里直接拷贝下来,能不改的地方尽量不改
     * @param message
     * @throws JMSException
     */
    public void  ackAndcloseSession(Message message) throws JMSException {
    	message.acknowledge();
    ...
    private class JmsTemplateResourceFactory implements ConnectionFactoryUtils.ResourceFactory {
        ...
        public Connection getConnection(JmsResourceHolder holder) {
            return ClientAckJmsTemplate.this.getConnection(holder);
        }
 
        @Override
        public Session getSession(JmsResourceHolder holder) {
            return ClientAckJmsTemplate.this.getSession(holder);
        }
 
        @Override
        public Connection createConnection() throws JMSException {
            return ClientAckJmsTemplate.this.createConnection();
        }
 
        @Override
        public Session createSession(Connection con) throws JMSException {
            return ClientAckJmsTemplate.this.createSession(con);
        }
 
        @Override
        public boolean isSynchedLocalTransactionAllowed() {
            return ClientAckJmsTemplate.this.isSessionTransacted();
        }
    }
