	...
	public void process(WatchedEvent event) {
		...
		if (Event.KeeperState.SyncConnected == keeperState) {
			...
			// 创建节点
				...
				System.out.println(logPrefix + path + "节点创建");
				try {
					Thread.sleep(100);
				} catch (InterruptedException e) {
					e.printStackTrace();
			...
			// 更新节点
				...
				System.out.println(logPrefix + path + "节点数据更新");
				try {
					Thread.sleep(100);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				System.out.println(logPrefix + path + "数据内容: " + this.readData(PARENT_PATH, true));
			}
			// 更新子节点
			else if (Event.EventType.NodeChildrenChanged == eventType) {
				System.out.println(logPrefix + path + "子节点变更");
				try {
					Thread.sleep(3000);
				} catch (InterruptedException e) {
					e.printStackTrace();
